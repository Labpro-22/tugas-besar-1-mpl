...
CoreGameSession::CoreGameSession(GameIO& io, TransactionLogger* logger)
    : io(io),
      logger(logger),
      turnManager(0),
      commandProcessor(
...
void CoreGameSession::startNewGame(const ConfigData& configData, const std::vector<std::string>& playerNames)
{
...
void CoreGameSession::loadGame(const ConfigData& configData, const GameState& gameState)
{
    this->configData = &configData;
    resetSessionState();
...
bool CoreGameSession::executeCommand(const Command& command)
{
    if (!gameReady) {
...
bool CoreGameSession::isReady() const
{
    return gameReady;
}

bool CoreGameSession::isGameEnded() const
{
    return gameEnded;
}

const ConfigData& CoreGameSession::getConfigData() const
{
...
const std::vector<Player>& CoreGameSession::getPlayers() const
{
    return players;
}

GameState CoreGameSession::snapshot() const
{
    return GameStateMapper::create(board, players, turnManager, skillDeck, logger);
}

std::vector<Player*> CoreGameSession::determineWinner() const
{
...
Player* CoreGameSession::getCurrentPlayer()
{
    return turnManager.getCurrentPlayer();
}

const Player* CoreGameSession::getCurrentPlayer() const
{
    return turnManager.getCurrentPlayer();
}

int CoreGameSession::getCurrentTurn() const
{
    return turnManager.getCurrentTurn();
}

int CoreGameSession::getMaxTurn() const
{
    return turnManager.getMaxTurn();
}

void CoreGameSession::setTurnChangedHandler(std::function<void()> handler)
{
    turnChangedHandler = std::move(handler);
}

bool CoreGameSession::ensureTurnPrepared()
{
...
bool CoreGameSession::beginCurrentTurn()
{
    Player* currentPlayer = getCurrentPlayer();
    if (currentPlayer == nullptr) {
...
bool CoreGameSession::finishTurnAndAdvance()
{
    if (checkGameEnd()) {
        gameEnded = true;
        return true;
...
bool CoreGameSession::checkGameEnd() const
{
    int activePlayers = 0;
    for (const Player& player : players) {
        if (!player.isBankrupt()) {
...
void CoreGameSession::rebuildContext()
{
    delete context;
...
void CoreGameSession::resetSessionState()
{
    delete context;
    context = nullptr;
...
void CoreGameSession::initializeBoardAndDecks()
{
...
void CoreGameSession::randomizeTurnOrder()
{
...
std::vector<Player*> CoreGameSession::buildPlayerPointers()
{
    return buildPlayerPointersFrom(players);
}

std::vector<Player*> CoreGameSession::buildPlayerPointers() const
{
